...
try {
    ...
    $stmt = $pdo->prepare("SELECT 
        vo.id, vo.payment_status, vo.visa_status, vo.total_amount, vo.currency, vo.created_at, vo.updated_at,
        c.country_name,
    ...
    if ($order) {
        echo json_encode([
            ...
            'updated_date' => date('F j, Y', strtotime($order['updated_at'] ?? $order['created_at']))
        ]);
    } else {
        echo json_encode([
            'success' => false,
